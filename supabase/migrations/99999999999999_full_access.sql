...
CREATE TABLE public.user_roles (
  ...
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE, -- Explicit ref to profiles
  role public.app_role NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (user_id, role)
);

-- 3. THE DEFINITIVE TRIGGER
...
DO $$ DECLARE t_name text; r_pol RECORD; BEGIN
